 ...
 * @constant {number}
 */
export const MAX_TOKEN_LIMIT = config.models.token_limit;
...
export const COMMAND_LIST = {
  start: {
    ...
    desc: "Tells the bot to join the voice channel you're currently in and begin recording the session. You can optionally specify participants using flags: `--gm @user` to set the game master (defaults to you), `--players @user @user ...` to list players to track, and `--trustees @user @user ...` to allow additional users to control the bot. Example: `@bot start --gm @gmUser --players @p1 @p2 --trustees @helper`.",
  },
  stop: {
    cmd: "stop",
...
export function ParseCommands(args, flags) {
  const flagSet = new Set(Object.values(flags));
...
export async function PromptModel(chatLog) {
  try {
    ...
    console.error(Red(`Could not prompt model due to error:`), err);
    throw err;
  }
}
